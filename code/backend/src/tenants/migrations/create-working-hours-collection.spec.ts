interface WorkingHoursMigration {
...
const migration =
  ...
  require('../../../migrations/20260706120200-create-working-hours-collection.js') as WorkingHoursMigration;
...
describe('workingHours collection migration contract', () => {
  describe('up', () => {
    it('applies a $jsonSchema validator requiring the core working hours fields', async () => {
    ...
    it('restricts weekday to the Weekday enum [mon..sun]', async () => {
      const db = buildDb({ collectionExists: false, existingIndexes: [] });

      await migration.up(db);

      expect(getAppliedSchema(db).properties.weekday.enum).toEqual([
        'mon',
        'tue',
        'wed',
        'thu',
    ...
    it('creates the unique uq_tenant_weekday index on { tenantId: 1, weekday: 1 }', async () => {
      const db = buildDb({ collectionExists: false, existingIndexes: [] });

      await migration.up(db);

      const [keys, opts] = findCreateIndexCall(db, UNIQUE_INDEX);
      expect(keys).toEqual({ tenantId: 1, weekday: 1 });
      expect(opts.unique).toBe(true);
    });
  });

  describe('down', () => {
    it('drops uq_tenant_weekday and clears the validator when the collection exists', async () => {
      const db = buildDb({
        collectionExists: true,
        existingIndexes: [UNIQUE_INDEX],
      });
