...
const migration =
  ...
  require('../../../migrations/20260706120500-create-customers-collection.js') as CustomersMigration;
...
describe('customers collection migration contract', () => {
  describe('up', () => {
    it('applies a $jsonSchema validator requiring the core customer fields', async () => {
    ...
    it('models phone as an optional string', async () => {
      const db = buildDb({ collectionExists: false, existingIndexes: [] });

      await migration.up(db);

      const schema = getAppliedSchema(db);
      expect(schema.properties.phone.bsonType).toBe('string');
  ...
  describe('down', () => {
    it('drops uq_tenant_email and clears the validator when the collection exists', async () => {
      const db = buildDb({
        collectionExists: true,
        existingIndexes: [EMAIL_INDEX],
      });
