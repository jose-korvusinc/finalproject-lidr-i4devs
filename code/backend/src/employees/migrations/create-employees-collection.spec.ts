...
const migration =
  ...
  require('../../../migrations/20260706120400-create-employees-collection.js') as EmployeesMigration;
...
describe('employees collection migration contract', () => {
  describe('up', () => {
    it('applies a $jsonSchema validator requiring the core employee fields', async () => {
      const db = buildDb({ collectionExists: false, existingIndexes: [] });

      await migration.up(db);

      expect(getAppliedSchema(db).required).toEqual(
        expect.arrayContaining(['tenantId', 'name', 'email', 'schemaVersion']),
      );
    });

    it('models serviceIds as an array of objectId', async () => {
      const db = buildDb({ collectionExists: false, existingIndexes: [] });

      await migration.up(db);

      const serviceIds = getAppliedSchema(db).properties.serviceIds;
      expect(serviceIds.bsonType).toBe('array');
      expect(serviceIds.items?.bsonType).toBe('objectId');
    });
    ...
    it('creates the unique uq_tenant_email index on { tenantId: 1, email: 1 }', async () => {
      const db = buildDb({ collectionExists: false, existingIndexes: [] });

      await migration.up(db);

      const [keys, opts] = findCreateIndexCall(db, EMAIL_INDEX);
      expect(keys).toEqual({ tenantId: 1, email: 1 });
      expect(opts.unique).toBe(true);
    });

    it('creates the idx_tenant_serviceIds index on { tenantId: 1, serviceIds: 1 }', async () => {
      const db = buildDb({ collectionExists: false, existingIndexes: [] });

      await migration.up(db);

      const [keys] = findCreateIndexCall(db, SERVICES_INDEX);
      expect(keys).toEqual({ tenantId: 1, serviceIds: 1 });
    });
  });
  ...
  describe('down', () => {
    it('drops both indexes and clears the validator when the collection exists', async () => {
      const db = buildDb({
        ...
        existingIndexes: [EMAIL_INDEX, SERVICES_INDEX],
      });

      await migration.down(db);

      expect(db.collectionMock.dropIndex).toHaveBeenCalledWith(EMAIL_INDEX);
      expect(db.collectionMock.dropIndex).toHaveBeenCalledWith(SERVICES_INDEX);
      const collMod = db.command.mock.calls.find(
        ([cmd]) => cmd.collMod === COLLECTION,
      );
